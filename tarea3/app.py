...
from database.db import SessionLocal, Comentario, Actividad
...
def index():
    ...
    return render_template("index.html", data=data, active_page="inicio")
...
def agregar_actividad():
    ...
    return render_template("agregar-actividad.html", active_page="agregar-actividad")
...
def listar_actividades():
    ...
    return render_template("listado-actividad.html", data=data, page=page, total=total, page_size=page_size, active_page="listar")
...
@app.route('/actividad/<int:activity_id>/comentario', methods=['POST'])
def agregar_comentario(activity_id):
    ...
    if error:
        session = SessionLocal()
        actividad = session.query(Actividad).get(activity_id)
        comentarios = session.query(Comentario).filter_by(actividad_id=activity_id).all()
        session.close()
        return render_template("detalle-actividad.html", actividad=actividad, comentarios=comentarios, error=error)
    
    session = SessionLocal()
    comentario = Comentario(nombre=nombre, texto=texto, actividad_id=activity_id)
    session.add(comentario)
    session.commit()
    session.close()
    return redirect(url_for("detalle_actividad", id=activity_id))
...
@app.route("/stats/actividades_por_dia", methods=["GET"])
@cross_origin(origins="127.0.0.1", supports_credentials=True)
def actividades_por_dia():
    data = db.get_activities_by_day()
    return jsonify(data)

@app.route("/stats/actividades_por_tipo", methods=["GET"])
@cross_origin(origins="127.0.0.1", supports_credentials=True)
def actividades_por_tipo():
    data = db.get_activities_by_type()
    return jsonify(data)

@app.route("/stats/actividades_por_horario", methods=["GET"])
@cross_origin(origins="127.0.0.1", supports_credentials=True)
